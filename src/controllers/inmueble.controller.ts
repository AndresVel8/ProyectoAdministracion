...
export class InmuebleController {
  ...
  @get('/inmuebles-por-sub-piso')
  @response(200, {
    description: 'consulta de listado de imuebles por sub piso',
    content: {
      'application/json': {
        schema: {
          type: 'array',
  ...
  async inmueblesDisponiblesParaArriendo(): Promise<Inmueble[]> {
    return this.inmuebleService.getInmueblesPorSubPiso();
  }
  ...
  @get('/inmueble-por-piso/{piso}')
  @response(200, {
    description: 'consulta de listado de imuebles por piso',
    content: {
      'application/json': {
        schema: {
          type: 'array',
  ...
  async inmueblesPorPiso(
    @param.path.string('piso') piso: string
  ): Promise<Inmueble[]> {
    return this.inmuebleService.getInmuebleporPiso(piso);
  }
