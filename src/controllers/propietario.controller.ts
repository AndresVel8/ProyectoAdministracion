...
export class PropietarioController {
  constructor(
    ...
    public autentificarService: AutentificacionService,
  ) { }
  ...
  async validarAcceso(
    ...
    let prop = await this.autentificarService.validarAcceso(credenciales.usuario, credenciales.clave);
    if (prop) {
      let token = this.autentificarService.generarTokenJWT(prop);
      return {
        datos: {
          nombre: `${prop.primer_nombre} ${prop.primer_apellido}`,
          correo: prop.correo,
          id: prop.id
        },
        token: token
  ...
  async create(
    ...
    // let contraseña = this.autentificarService.generarClave();
    propietario.contrasena = this.autentificarService.ocultarClave(propietario.contrasena);
    let prop = await this.propietarioRepository.create(propietario);

    fetch('http://localhost:5000/enviar?mensaje=Prueba de notificacion&asunto=pruebas&correo=' + prop.correo)
      .then(response => response.text())
      .then(data => console.log(`Esta es la respuesta del servcio ${data}`));
    return prop;
  }
